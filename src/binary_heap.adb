...
package body Binary_Heap is
   ...
   is
      Top_Item : Item_Weight;
   begin
      Top_Item := Heap.Nodes (1);
      Top_Elem := Top_Item.Item;
      Top_Weight := Top_Item.Weight;
   ...
   begin
      Heap.Length := Heap.Length + 1;
      Heap.Nodes (Heap.Length) := (New_Elem, New_Weight);
   ...
   begin
      ...
      then
         ...
         if Left > 0 and then
            ...
            Right <= Heap.Length
         then
            if Heap.Nodes (Left).Weight <
               Heap.Nodes (Changed_Pos).Weight
            then
               if Heap.Nodes (Right).Weight < Heap.Nodes (Left).Weight then
   ...
   begin
      ...
      if Parent in 1 .. Heap.Length and then
         ...
         Heap.Nodes (Changed_Pos).Weight < Heap.Nodes (Parent).Weight
      then
